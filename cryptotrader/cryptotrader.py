...
class Cryptotrader(object):
    ...
    def __init__(self, market, strategy, interval=timedelta(minutes=1), action='BUY'):
    ...
    def test(self, action='BUY'):
        ...
        for i in range(0, steps):
            ...
            if curr_action == 'BUY':
                buy_price = self.strategy.enter(sliced_data)
                if tick_low <= buy_price <= tick_high:
                    trades.append({'timestamp': curr_tick, 'action': curr_action, 'price': buy_price})
                    curr_action = 'SELL'
            else:
                sell_price = self.strategy.exit(sliced_data)
